...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, masks=None):
        if isinstance(x, list):
            raise NotImplementedError("forward_features does not support list input")
    ...
    def _add_token_injection(self, x, inj):
        """Add [B, N_patches, D] injection to the patch tokens (cls / register tokens untouched)."""
        offset = 1 + self.num_register_tokens
        return torch.cat([x[:, :offset], x[:, offset:] + inj.to(x.dtype)], dim=1)

    def _get_intermediate_layers_not_chunked(self, x, n=1, token_injections=None):
        x = self.prepare_tokens_with_masks(x)
        # If n is an int, take the n last blocks. If it's a list, take them
        output, total_block_len = [], len(self.blocks)
        blocks_to_take = range(total_block_len - n, total_block_len) if isinstance(n, int) else n
        for i, blk in enumerate(self.blocks):
            if token_injections is not None and i in token_injections:
                x = self._add_token_injection(x, token_injections[i])
            x = blk(x)
    ...
    def _get_intermediate_layers_chunked(self, x, n=1, token_injections=None):
        x = self.prepare_tokens_with_masks(x)
    ...
    def get_intermediate_layers(
        ...
        norm=True,
        token_injections: Optional[Dict[int, torch.Tensor]] = None,
    ) -> Tuple[Union[torch.Tensor, Tuple[torch.Tensor]]]:
        if self.chunked_blocks:
            outputs = self._get_intermediate_layers_chunked(x, n, token_injections)
        else:
            outputs = self._get_intermediate_layers_not_chunked(x, n, token_injections)
        if norm:
