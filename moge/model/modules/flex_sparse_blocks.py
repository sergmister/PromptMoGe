...
def _pick_spconv_algorithm(in_channels: int, out_channels: int) -> str:
    """Pick the submanifold-conv index-GEMM variant.
    Default to 'implicit_gemm' which is verified numerically stable across forward
    and backward passes on all GPU architectures. Can be overridden via MOGE_SPCONV_ALGORITHM.
    """
    import os
    override = os.environ.get("MOGE_SPCONV_ALGORITHM")
    if override:
        return override
