...
class ConvStack(nn.Module):
    def __init__(self, 
        ...
        # `output_kernel` > 1 is used by the 4-level (240x320) ladder: its last level has to reproduce what the
        # 5-level ladder computed with an upsample + 3x3 convolution, which a 1x1 projection cannot express.
        self.output_blocks = nn.ModuleList([
            nn.Conv2d(dim_res_block_, dim_out_, kernel_size=output_kernel, stride=1, padding=output_kernel // 2,
                      padding_mode='replicate' if output_kernel > 1 else 'zeros') if dim_out_ is not None else nn.Identity() 
    ...
    def forward(self, in_features: List[torch.Tensor], injections: Optional[List[Optional[torch.Tensor]]] = None):
        out_features = []
        for i in range(len(self.res_blocks)):
            ...
            if injections is not None and injections[i] is not None:
                x = x + injections[i].to(x.dtype)     # zero-init prompt injection (LiDAR fusion)
