...
class DINOv2Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, token_injections: Optional[Dict[int, torch.Tensor]] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Get intermediate layers from the backbone (optionally with additive LiDAR prompt token injections before given blocks)
        features = self.backbone.get_intermediate_layers(image_14, n=self.intermediate_layers, return_class_token=True, token_injections=token_injections)
