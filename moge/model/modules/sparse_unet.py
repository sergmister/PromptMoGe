...
class Sparse3DUNet(nn.Module):
    ...
    def __init__(
        ...
        **deprecated_kwargs,
        ...
        super().__init__()
        # Optional zero-initialised per-voxel prompt channel (confidence-gated LiDAR log-residual).
        ...
        if prompt_channels > 0:
            self.prompt_proj = nn.Linear(prompt_channels, model_channels[0])
            nn.init.zeros_(self.prompt_proj.weight); nn.init.zeros_(self.prompt_proj.bias)
    ...
    def forward(
        ...
        if prompt_feats is not None:
            assert self.prompt_channels > 0, "refiner was built without prompt_channels"
            feats = feats + self.prompt_proj(prompt_feats.to(feats.dtype))
        ...
        if encoder_bias_map is not None:
            # the UV planes' contribution to `encoder_fuse` is a fixed function of the pixel
            # position, so it is precomputed as a [B, C, h, w] map and added here instead of widening the
            # GEMM's contraction from 1024 to 1026 (which pads to 1152 on the ANE).
            enc_feat = enc_feat + self._sample_encoder_feature(encoder_bias_map, coords).to(enc_feat.dtype)
