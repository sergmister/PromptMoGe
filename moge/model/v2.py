...
class MoGeModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        # A deliberately re-shaped module (the compressed refiner variants: a narrowed level 4, the UV planes folded out
        # of `encoder_fuse`) cannot take the base tensor; load_state_dict raises on a size mismatch even with
        # strict=False, so those keys are dropped here and reported loudly -- they are re-initialised on purpose
        # and are then filled in by the fine-tuned overlay via `load_trainable_state`.
        sd_ck = dict(checkpoint['model'])
        own = model.state_dict()
        shape_mismatch = [k for k, v in sd_ck.items() if k in own and tuple(own[k].shape) != tuple(v.shape)]
        for k in shape_mismatch:
            sd_ck.pop(k)
        if shape_mismatch:
            warnings.warn(f"{len(shape_mismatch)} checkpoint tensor(s) have a different shape in this model "
                          f"configuration and were NOT loaded: {shape_mismatch}")
        missing_keys, unexpected_keys = model.load_state_dict(sd_ck, strict=False)
        missing_keys = [k for k in missing_keys if k not in shape_mismatch]
        if missing_keys:
            warnings.warn(
